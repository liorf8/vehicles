...
public class SimulatonEngine extends PApplet {
    ...
    public SimulatonEngine(Simulation simu) {
        ...
        for (int i = 0; i < this.num_sources; i++) {
            ...
            elementVector.add(new Source(
                    ...
                    curr.getType()));
            print(elementVector.elementAt(i));
        }
    }
        ...
        for (int i = 0; i < width; i += px) {
            for (int k = 0; k < height; k += px) { //process every pixel in the image
                ...
                for (int m = 0; m < num_sources; m++) {
                    ...
                    sum += elementVector.elementAt(m).getReading(i, k, true);
    ...
    class Source {
        ...
        void checkCollision() {
            ...
            float dx, dy, da;
            Iterator<Source> elementIterator = elementVector.iterator();
            while (elementIterator.hasNext()) {
                Source temp = elementIterator.next();
                ...
                if (temp.id != id) {
                    ...
                    if (dx * dx + dy * dy < max_radius) {
                        da = atan2(dy, dx);
                        temp.x = temp.x + cos(da + PI) * 5;
                        temp.y = temp.y + sin(da + PI) * 5;
                    }
                }
            }
        }
